...
function objectExpression(expression, sourceFile) {
  ...
  if (Node.isIdentifier(expression)) {
    ...
    if (localDeclaration) return objectExpression(localDeclaration.getInitializer(), sourceFile);
  ...
  function entries(propertyName) {
    ...
    return routes.getProperties().flatMap((routeProperty) => {
      ...
      return [{
        ...
        path,
        route,
        sourceFile: route.getSourceFile(),
        module,
        package: packageInfo,
      }];
    });
...
function pageSource(page, filePath) {
  ...
  const eventRegistration = relativeImport(filePath, join(webRoot, ".generated", "events", "register.ts"));
  const importLine = [
    `import ${JSON.stringify(eventRegistration)};`,
    page.module.importName === "packageDefinition"
