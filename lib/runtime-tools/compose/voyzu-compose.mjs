...
async function validatePackageSurfaces(packages, label) {
  ...
  for (const packageInfo of packages) {
    for (const [rootPath, root] of Object.entries(packageInfo.pageRouting?.roots ?? {})) for (const [id, route] of Object.entries(root.routes)) {
      routes.push({ packageName: packageInfo.name, route: { ...route, id, rootPath } });
    }
    ...
    if (navigation.topNav !== undefined) {
      ...
      if (!routes.some(({ packageName, route }) => packageName === packageInfo.name && route.id === navigation.topNav.routeId)) {
        throw new Error(`${packageInfo.name}: top navigation must reference a route in one of its roots.`);
      }
    }
    if (navigation.domains !== undefined) {
      ...
      for (const domain of navigation.domains) {
        if (
          ...
          || typeof domain.rootPath !== "string"
          || !Object.hasOwn(packageInfo.pageRouting?.roots ?? {}, domain.rootPath)
        ) {
          throw new Error(`${packageInfo.name} navigation contains an invalid domain.`);
        }
        navigationRouteIds.push(domain.routeId);
        if (navigationRoots.has(domain.rootPath)) {
          throw new Error(`${packageInfo.name}: multiple navigation domains select root ${domain.rootPath}.`);
        }
        navigationRoots.add(domain.rootPath);
        if (!Object.hasOwn(packageInfo.pageRouting.roots[domain.rootPath].routes, domain.routeId)) {
          throw new Error(`${packageInfo.name}: navigation default route must belong to root ${domain.rootPath}.`);
        }
  ...
  for (const { packageName, route } of routes) {
    ...
    if (routePaths.has(pagePattern(route.path))) {
      throw new Error(`Duplicate ${label} page route path: ${route.path}.`);
    }
    routeIds.add(route.id);
    routePaths.add(pagePattern(route.path));
  }
...
export function create${exportPrefix[0].toUpperCase()}${exportPrefix.slice(1)}PackageDomains(
  additionalPageRoutes: readonly RegisteredPageRoute[] = [],
): VoyzuComposedSurfaceDomain[] {
  ...
  ({ packageName, domains, topNav, leftNav }) => {
    ...
    if (domains) {
      ...
      return domains.map((domain) => {
        ...
        const domainRoutes = pageRoutes.filter((route) => route.rootPath === domain.rootPath);
        return {
          id: domain.routeId,
          packageName,
          label: domain.label,
    ...
    return [{
      ...
      label: topNav.label,
      defaultPath: defaultRoute.path,
      routePaths: pageRoutes.filter((route) => route.rootPath === defaultRoute.rootPath).map(({ id, path }) => ({ id, path })),
