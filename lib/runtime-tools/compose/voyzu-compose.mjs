...
Options:
  ...
  --surfaces-only         Refresh HTTP API, page/navigation registries and HTTP documentation only.
...
function parseArgs(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    } else if (argument === "--surfaces-only") {
...
async function packageDescriptor(manifest, directory, root, source) {
  ...
  const descriptor = {
    ...
    helpBaseUrl,
    pageRootPaths: pageRouting?.roots ?? [],
...
async function validatePackageSurfaces(packages, label) {
  ...
  for (const packageInfo of packages) {
    for (const [id, route] of Object.entries(packageInfo.pageRouting?.routes ?? {})) {
      routes.push({ packageName: packageInfo.name, route: { ...route, id } });
    }
...
function generatedPageRoutes(packages, exportPrefix) {
  ...
  return '// Generated by voyzu compose. Do not edit.\n'
    ...
    + '  Object.entries(definition.contracts?.pageRouting?.routes ?? {}).map(([id, route]) => ({\n'
    + '    ...route, id, packageName, helpBaseUrl: helpBaseUrl ?? undefined,\n'
...
async function main() {
  ...
  if (!cliOptions.surfacesOnly) await writeGeneratedContracts(allPackages);
  ...
  if (cliOptions.surfacesOnly) {
    ...
    console.log("HTTP API and surface composition complete.");
    return;
  }
