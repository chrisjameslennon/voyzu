...
const generatedEventsRoot = join(webRoot, ".generated", "events");
const generatedRoutesRoot = join(webRoot, "app", "(generated)");
const routeGenerator = join(runtimeRoot, "lib", "runtime-tools", "compose", "generate-next-routes.mjs");
...
async function isInstalledRuntime() {
  const runtimeWorkspaceRoot = dirname(runtimeRoot);
  const instanceRoot = dirname(runtimeWorkspaceRoot);
  const instanceManifestPath = join(instanceRoot, "package.json");
  if (
    runtimeRoot !== join(runtimeWorkspaceRoot, "voyzu")
    || !(await pathExists(instanceManifestPath))
    || !(await pathExists(join(runtimeWorkspaceRoot, "packages")))
  ) {
    return false;
  }
  const manifest = await readJson(instanceManifestPath);
  return manifest.voyzu?.mode === "development" || manifest.voyzu?.mode === "production";
}
...
async function discoverPackages(selectedPackageNames) {
  ...
  for (const directory of await candidatePackageDirectories()) {
    ...
    packages.push({
      ...
      hasListeners: hasExport(manifest, "./listeners"),
      eventModules: Object.keys(manifest.exports ?? {}).flatMap((exportName) => {
        const match = exportName.match(/^\.\/([^/]+)\/events$/);
...
function generatedSurfaceRegistry(packages) {
  ...
  const registrations = packages.map(
    ({ name, hasDomains, hasTopNav, hasLeftNav, helpBaseUrl, pageRootPaths }, index) =>
      `{ packageName: ${JSON.stringify(name)}, definition: packageDefinition${index}, pageRootPaths: ${JSON.stringify(pageRootPaths)}, helpBaseUrl: ${
...
type ComposedPackageRegistration = {
  ...
  readonly pageRootPaths: readonly string[];
  readonly definition: {
      ...
      return (Object.values(moduleDefinition.pageRoutes) as VoyzuSurfaceRoute[]).map(
        (route) => {
          ...
          if (!pageRootPaths.some((rootPath) => route.path === rootPath || route.path.startsWith(rootPath + "/"))) {
            throw new Error("Package " + packageName + " page route " + route.path + " is outside its declared page root paths.");
          }
...
export const installedPackagePageRoutes: VoyzuSurfaceRoute[] = packages.flatMap(
  ({ packageName, pageRootPaths, definition, helpBaseUrl }) => packagePageRoutes(packageName, pageRootPaths, definition, helpBaseUrl),
);
...
export function createInstalledPackageDomains(
  ...
  ({ packageName, pageRootPaths, definition, helpBaseUrl, domains, topNav, leftNav }) => {
    const pageRoutes = packagePageRoutes(packageName, pageRootPaths, definition, helpBaseUrl);
    if (domains) {
...
function generatedEventRegistration(packages) {
  const eventRegistrations = packages.flatMap(({ name, eventModules }, packageIndex) =>
    eventModules.map((moduleName, moduleIndex) => ({
      importName: `packageEvents${packageIndex}_${moduleIndex}`,
      importPath: `${name}/${moduleName}/events`,
      packageName: name,
      moduleName,
    })),
  );
  const listenerPackages = packages.flatMap(({ name, hasListeners }, packageIndex) =>
    hasListeners ? [{ importName: `packageListeners${packageIndex}`, importPath: `${name}/listeners` }] : [],
  );
  const imports = [
    ...eventRegistrations.map(({ importName, importPath }) =>
      `import { events as ${importName} } from ${JSON.stringify(importPath)};`),
    ...listenerPackages.map(({ importName, importPath }) =>
      `import ${importName} from ${JSON.stringify(importPath)};`),
  ];
  const eventRegistrationLines = eventRegistrations.map(({ importName, packageName, moduleName }) =>
    `platformEvents.registerModule(${JSON.stringify(packageName)}, ${JSON.stringify(moduleName)}, ${importName});`);
  const listenerRegistrationLines = listenerPackages.map(({ importName }) =>
    `for (const listener of ${importName}) platformEvents.listen(listener.event, listener.handler);`);
...
const eventGlobal = globalThis as typeof globalThis & {
  __voyzuListenersRegistered?: boolean;
};

${eventRegistrationLines.join("\n")}
if (!eventGlobal.__voyzuListenersRegistered) {
  ${listenerRegistrationLines.join("\n  ")}
  eventGlobal.__voyzuListenersRegistered = true;
}
`;
}

async function writeGeneratedEvents(packages) {
  await mkdir(generatedEventsRoot, { recursive: true });
  await writeFile(
    join(generatedEventsRoot, "register.ts"),
    generatedEventRegistration(packages),
    "utf8",
  );
}
...
async function main() {
  ...
  if (cliOptions.emptyIfMissing) {
    ...
    const eventRegistrationPath = join(generatedEventsRoot, "register.ts");
    if (!(await pathExists(compositionPath))) {
      await Promise.all([writeGeneratedNavigation([]), writeGeneratedEvents([])]);
    ...
    if (!(await pathExists(eventRegistrationPath))) {
      await writeGeneratedEvents([]);
      console.log("Created empty Voyzu package event registry.");
    }
    if (!(await isInstalledRuntime()) || !(await pathExists(generatedRoutesRoot))) {
      await run(process.execPath, [routeGenerator, "--runtime", runtimeRoot], { cwd: runtimeRoot });
    }
    return;
  }
  ...
  await Promise.all([writeGeneratedNavigation(packages), writeGeneratedEvents(packages)]);
