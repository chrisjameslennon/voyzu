...
async function packageDescriptor(manifest, directory, root, source) {
  ...
  const descriptor = {
    ...
    operationModules: exportedModules(manifest, "operations"),
...
async function writeGeneratedOperations(packages, fileName) {
  await mkdir(generatedOperationsRoot, { recursive: true });
  await writeFile(
    join(generatedOperationsRoot, fileName),
...
async function removeLegacyGeneratedRegistries() {
  await Promise.all([
    ...
    rm(join(generatedOperationsRoot, "preinstalled.ts"), { force: true }),
    rm(join(generatedOperationsRoot, "register.ts"), { force: true }),
  ]);
}
...
async function main() {
  ...
  if (cliOptions.emptyIfMissing) {
    ...
    await Promise.all([
      ...
      writeGeneratedOperations(preInstalledPackages, "pre-installed.ts"),
    ...
    const installedOperationsPath = join(generatedOperationsRoot, "installed.ts");
    ...
    if (
      ...
      || !(await pathExists(installedOperationsPath))
      ...
      await Promise.all([
        ...
        writeGeneratedOperations([], "installed.ts"),
  ...
  await Promise.all([
    ...
    writeGeneratedOperations(packages, "installed.ts"),
