...
function generatedPageRoutes(packages, exportPrefix) {
  const registrations = packages.flatMap(({ name, pageRouteModules, helpBaseUrl }) =>
  ...
  const modules = registrations.map(({
    ...
    }, httpApiDocsUrl: "/http-api-reference", routes: Object.values(${importName}) as VoyzuSurfaceRoute[] },`);
...
export const ${exportPrefix}PageRouteModules: {
  ...
  httpApiDocsUrl: string | undefined;
...
export const ${exportPrefix}PageRoutes: VoyzuSurfaceRoute[] =
  ${exportPrefix}PageRouteModules.flatMap(({ packageName, helpBaseUrl, httpApiDocsUrl, routes }) =>
    routes.map((route) => ({
      ...route,
      packageName,
      helpBaseUrl,
