...
function generatedSurfaceRegistry(packages) {
  ...
  const registrations = packages.map(
    ({ name, hasDomains, hasTopNav, hasLeftNav, helpBaseUrl, pageRootPaths, apiRootPaths }, index) =>
      `{ packageName: ${JSON.stringify(name)}, definition: packageDefinition${index}, pageRootPaths: ${JSON.stringify(pageRootPaths)}, apiRootPaths: ${JSON.stringify(apiRootPaths)}, helpBaseUrl: ${
...
type ComposedPackageRegistration = {
  ...
  readonly apiRootPaths: readonly string[];
  readonly definition: {
...
function packageApiModules(
  packageName: string,
  apiRootPaths: readonly string[],
  definition: ComposedPackageRegistration["definition"],
): VoyzuApiModule[] {
  return definition.modules.map((moduleDefinition) => {
    for (const apiDefinition of Object.values(moduleDefinition.apiDefinitions) as { readonly path: string }[]) {
      if (!apiRootPaths.some((rootPath) => apiDefinition.path === rootPath || apiDefinition.path.startsWith(rootPath + "/"))) {
        throw new Error("Package " + packageName + " API route " + apiDefinition.path + " is outside its declared API root paths.");
      }
    }
    return moduleDefinition as VoyzuApiModule;
  });
}

export const installedPackageApiModules: VoyzuApiModule[] = packages.flatMap(
  ({ packageName, apiRootPaths, definition }) => packageApiModules(packageName, apiRootPaths, definition),
);
