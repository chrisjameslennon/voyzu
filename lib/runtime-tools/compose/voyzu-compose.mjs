...
const generatedComponentsRoot = join(webRoot, ".generated", "components");
...
async function packageDescriptor(manifest, directory, root, source) {
  ...
  const descriptor = {
    ...
    clientComponentModules: exportedModules(manifest, "client-components"),
...
async function writeGeneratedContracts(packages) {
  await run(process.execPath, [
    ...
    "--import", "tsx",
    join(runtimeRoot, "lib/runtime-tools/compose/compose-contracts.ts"),
...
function generatedComponentRegistration(packages) {
  const registrations = packages.flatMap(({ name, componentModules }, packageIndex) =>
...
async function writeGeneratedComponents(packages, fileName) {
  await mkdir(generatedComponentsRoot, { recursive: true });
  await writeFile(
    join(generatedComponentsRoot, fileName),
    generatedComponentRegistration(packages),
    "utf8",
  );
}

async function writeGeneratedClientComponents(packages) {
  const owners = new Map();
  const imports = [];
  const entries = [];
  for (const pkg of packages) {
    for (const moduleName of pkg.clientComponentModules) {
      const path = `${pkg.name}/${moduleName}/client-components`;
      // Declaration modules must contain only metadata and lazy loaders, never client imports.
      const { clientComponents } = await tsImport(path, import.meta.url);
      if (!clientComponents || typeof clientComponents !== "object") throw new Error(`${path} must export clientComponents`);
      for (const definition of Object.values(clientComponents)) {
        if (!definition || typeof definition.id !== "string" || !definition.id || typeof definition.loadComponent !== "function") throw new Error(`Invalid client component declaration in ${path}`);
        if (owners.has(definition.id)) throw new Error(`Duplicate client component ${definition.id}: ${owners.get(definition.id)} and ${path}`);
        owners.set(definition.id, path);
      }
      const alias = `clientComponents${imports.length}`;
      imports.push(`import { clientComponents as ${alias} } from ${JSON.stringify(path)};`);
      entries.push(`...Object.values(${alias})`);
    }
  }
  await mkdir(generatedComponentsRoot, { recursive: true });
  await writeFile(join(generatedComponentsRoot, "client.tsx"), `// Generated by voyzu compose. Do not edit.
"use client";
import type { ComponentType, ReactNode } from "react";
import { ClientComponentProvider } from "@voyzu/ui-surface/client";
${imports.join("\n")}
const registrations: readonly { id: string; loadComponent: () => Promise<ComponentType<any>> }[] = [${entries.join(", ")}];
...
async function main() {
  ...
  await writeGeneratedClientComponents(allPackages);
  await writeGeneratedContracts(allPackages);
  ...
  await Promise.all([
    ...
    writeGeneratedComponents(preInstalledPackages, "pre-installed.ts"),
    ...
    writeGeneratedComponents(packages, "installed.ts"),
