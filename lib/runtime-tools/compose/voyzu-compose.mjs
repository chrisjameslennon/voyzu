...
async function validatePreinstalledSurfaces(packages) {
  const routes = [];
  const apiRoutes = [];
  ...
  for (const { packageName, route } of routes) {
    if (routeIds.has(route.id)) {
      throw new Error(`Duplicate preinstalled page route id: ${route.id}.`);
    }
    if (routePaths.has(route.path)) {
      throw new Error(`Duplicate preinstalled page route path: ${route.path}.`);
    }
  ...
  for (const routeId of navigationRouteIds) {
    if (typeof routeId !== "string" || !routeIds.has(routeId)) {
      throw new Error(`Preinstalled navigation route id was not found: ${String(routeId)}.`);
    }
  }
  ...
  for (const route of apiRoutes) {
    ...
    if (apiRouteKeys.has(key)) {
      throw new Error(`Duplicate preinstalled API route: ${key}.`);
    }
...
function generatedPreinstalledApiRoutes(packages) {
  ...
  const routes = registrations.map(({ importName }) =>
    `  ...(Object.values(${importName}) as VoyzuApiModuleRoute[]),`);
    ...
    (moduleDefinition) => {
      return (Object.values(moduleDefinition.pageRoutes) as VoyzuSurfaceRoute[]).map(
        (route) => {
          ...
          return { ...route, packageName, helpBaseUrl };
        },
      );
    },
  );
...
async function main() {
  ...
  await validatePreinstalledSurfaces(preinstalledPackages);
  await writeGeneratedPreinstalledSurfaces(preinstalledPackages);
