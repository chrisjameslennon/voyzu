...
async function main() {
  ...
  if (cliOptions.emptyIfMissing) {
    ...
    const installedComponentsPath = join(generatedComponentsRoot, "installed.ts");
    ...
    if (
      ...
      || !(await pathExists(installedComponentsPath))
      ...
      await Promise.all([
        ...
        writeGeneratedComponents([], "installed.ts"),
        writeGeneratedHeaders([], "installed-headers.tsx"),
      ]);
      console.log("Created empty installed-package registries.");
    }
    if (!(await pathExists(join(webRoot, ".generated", "contracts", "installed.ts")))) await writeGeneratedContracts(preInstalledPackages);
