...
function startupElapsed() {
  const seconds = Math.floor((performance.now() - startupStarted) / 1000);
  return `elapsed ${Math.floor(seconds / 60)}:${String(seconds % 60).padStart(2, "0")}`;
}
...
async function main() {
  ...
  console.log(`[voyzu] development package discovery loaded in ${Math.round(performance.now() - sectionStarted)} ms ${startupElapsed()}`);
  ...
  console.log(`[voyzu] development package copies loaded in ${Math.round(performance.now() - sectionStarted)} ms ${startupElapsed()}`);
  sectionStarted = performance.now();
  console.log("[voyzu] loading package watchers ...");
  const watchers = await Promise.all(mirrors.map(startPackageWatcher));
  console.log(`[voyzu] package watchers loaded in ${Math.round(performance.now() - sectionStarted)} ms ${startupElapsed()}`);
  ...
  const forwardLine = (line, destination) => {
    ...
    // Child processes use the same startup clock through this output handler.
    const completed = /^\[voyzu\] .+ loaded in \d+ ms$/.test(plainLine);
    destination.write(`${line}${completed ? ` ${startupElapsed()}` : ""}\n`);
    if (readyReported) return;
    clearTimeout(readyTimer);
    const message = stripVTControlCharacters(line).trim().replace(/^\[voyzu\]\s*/, "");
    if (!listening && /\bReady in\s+\d/.test(message)) {
      listening = true;
      console.log(`[voyzu] web server loaded in ${Math.round(performance.now() - webStarted)} ms ${startupElapsed()}`);
    }
    ...
    if (listening && contractsLoaded && !configurationLoading) {
      readyTimer = setTimeout(() => {
        ...
        readyReported = true;
        console.log(`[voyzu] ✓ Ready for first web server call ${startupElapsed()}`);
      }, 1000);
    }
  };
