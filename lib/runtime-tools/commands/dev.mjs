...
async function main() {
  ...
  console.log("loading development package discovery ...");
  const context = await findDevelopmentInstance();
  const mirrors = await discoverDevelopmentMirrors(context);
  console.log(`development package discovery loaded in ${Math.round(performance.now() - sectionStarted)} ms`);
  sectionStarted = performance.now();
  console.log("loading development package copies ...");
  for (const mirror of mirrors) await mirrorPackage(mirror);
  console.log(`development package copies loaded in ${Math.round(performance.now() - sectionStarted)} ms`);
  sectionStarted = performance.now();
  console.log("loading package watchers ...");
  const watchers = await Promise.all(mirrors.map(startPackageWatcher));
  console.log(`package watchers loaded in ${Math.round(performance.now() - sectionStarted)} ms`);
  ...
  // Next's listening notification is earlier than instrumentation and first-route compilation.
  let readyReported = false;
  const forwardLine = (line, destination) => {
    destination.write(`${line}\n`);
    if (!readyReported && /\bReady in\s+\d/.test(stripVTControlCharacters(line))) {
      readyReported = true;
      console.log(`web server loaded in ${Math.round(performance.now() - webStarted)} ms`);
