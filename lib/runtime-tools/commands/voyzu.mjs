  ...
  for (let leftIndex = 0; leftIndex < packages.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < packages.length; rightIndex += 1) {
      ...
      for (const property of ["pageRootPaths", "httpApiRoots"]) {
        for (const leftPath of (property === "httpApiRoots" ? left.httpApiRoots : left.manifest.voyzu[property])) {
          const rightPath = (property === "httpApiRoots" ? right.httpApiRoots : right.manifest.voyzu[property]).find((path) => rootPathsCollide(leftPath, path));
          if (rightPath) {
            throw new Error(
