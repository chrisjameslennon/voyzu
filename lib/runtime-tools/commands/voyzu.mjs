...
async function validateInstallablePackageManifest(
  ...
  validateRootPaths(packageName, "pageRouting.roots", Object.keys(definition.contracts?.pageRouting?.roots ?? {}));
...
async function assertUniquePackageRootPaths(context, selectedPackages) {
  ...
  for (const packageInfo of packages) {
    ...
    packageInfo.pageRootPaths = Object.keys(definition.contracts?.pageRouting?.roots ?? {});
...
async function compose(context, { noInstall = false, acceptMissingImplementations = false, routingOnly = false } = {}) {
...
async function main() {
  ...
  if (command === "compose") {
    ...
    if (flags.some(flag => !["--no-install", "--accept-missing-implementations", "--routing-only"].includes(flag))) {
      throw new Error("Usage: npm run voyzu:compose [-- --no-install] [--accept-missing-implementations] [--routing-only]");
    }
    await compose(context, {
      noInstall: flags.includes("--no-install"),
      routingOnly: flags.includes("--routing-only"),
