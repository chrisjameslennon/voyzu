...
Usage:
  ...
  npm run voyzu:initialize [-- --no-build]
...
Commands:
  ...
  initialize   Initialize the preinstalled Voyzu platform; --no-build skips the build
...
async function validateInstallablePackageManifest(
  ...
  validateRootPaths(packageName, "pageRouting.roots", definition.contracts?.pageRouting?.roots ?? []);
  validateRootPaths(packageName, "httpApiRouting.roots", definition.contracts?.httpApiRouting?.roots ?? []);
}
...
async function assertUniquePackageRootPaths(context, selectedPackages) {
  ...
  for (const packageInfo of packages) {
    const { default: definition } = await tsImport(pathToFileURL(join(packageInfo.directory, "voyzu.package.ts")).href, import.meta.url);
    packageInfo.pageRootPaths = definition.contracts?.pageRouting?.roots ?? [];
    validateRootPaths(packageInfo.name, "pageRouting.roots", packageInfo.pageRootPaths);
    packageInfo.httpApiRoots = definition.contracts?.httpApiRouting?.roots ?? [];
    validateRootPaths(packageInfo.name, "httpApiRouting.roots", packageInfo.httpApiRoots);
  }
  for (let leftIndex = 0; leftIndex < packages.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < packages.length; rightIndex += 1) {
      ...
      for (const property of ["pageRootPaths", "httpApiRoots"]) {
...
async function refreshVoyzu(context) {
  ...
  // generated HTTP API documentation. They are disposable runtime output; any
  // other platform edit remains protected and prevents refresh.
  const generatedRuntimePaths = [
    "apps/web/.generated/http-api-reference",
...
async function compose(context, { noInstall = false, acceptMissingImplementations = false, surfacesOnly = false } = {}) {
  ...
  await run(
    ...
    [
      ...
      ...(surfacesOnly ? ["--surfaces-only"] : []),
      ...(acceptMissingImplementations ? ["--accept-missing-implementations"] : []),
    ],
...
async function main() {
  ...
  if (command === "initialize") {
    ...
    if (options.some((option) => option !== "--no-build")) {
      throw new Error("Usage: npm run voyzu:initialize [-- --no-build]");
    }
    await installPreinstalledPackages(context);
    if (!options.includes("--no-build")) {
      await compose(context, { noInstall: true });
      console.log("Building Voyzu...");
  ...
  if (command === "compose") {
    const flags = [subject, ...rest].filter(Boolean);
    if (flags.some(flag => !["--no-install", "--accept-missing-implementations", "--surfaces-only"].includes(flag))) {
      throw new Error("Usage: npm run voyzu:compose [-- --no-install] [--accept-missing-implementations] [--surfaces-only]");
    }
    await compose(context, {
      noInstall: flags.includes("--no-install"),
      surfacesOnly: flags.includes("--surfaces-only"),
      acceptMissingImplementations: flags.includes("--accept-missing-implementations"),
    });
    return;
  }
