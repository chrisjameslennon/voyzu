...
function replacePackageTemplateText(value, packageName, packageSlug) {
  return value.replace(
    /@voyzu\/ice-creams|ice-creams|ice-cream/g,
    (match) => match === "@voyzu/ice-creams" ? packageName : packageSlug,
  );
}

async function rewritePackageTemplateContents(directory, packageName, packageSlug) {
  const entries = await readdir(directory, { withFileTypes: true });
  for (const entry of entries) {
    const path = join(directory, entry.name);
    if (entry.isDirectory()) {
      await rewritePackageTemplateContents(path, packageName, packageSlug);
      continue;
    }
    ...
    const replaced = replacePackageTemplateText(text, packageName, packageSlug);
    if (replaced !== text) await writeFile(path, replaced, "utf8");
  }
}

async function rewritePackageTemplatePaths(directory, packageSlug) {
  const entries = await readdir(directory, { withFileTypes: true });
  for (const entry of entries) {
    const path = join(directory, entry.name);
    if (entry.isDirectory()) await rewritePackageTemplatePaths(path, packageSlug);

    const nextName = replacePackageTemplateText(entry.name, packageSlug, packageSlug);
...
async function createPackage(context, requestedName) {
  ...
  const [, packageSlug] = packagePathParts(packageName);
  ...
  try {
    ...
    await rewritePackageTemplateContents(target, packageName, packageSlug);
    await rewritePackageTemplatePaths(target, packageSlug);
