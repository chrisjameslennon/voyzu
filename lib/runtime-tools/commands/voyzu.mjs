...
async function compose(context, { noInstall = false, acceptMissingImplementations = false } = {}) {
...
async function main() {
  ...
  if (command === "compose") {
    ...
    if (flags.some(flag => !["--no-install", "--accept-missing-implementations"].includes(flag))) {
      throw new Error("Usage: npm run voyzu:compose [-- --no-install] [--accept-missing-implementations]");
    }
    await compose(context, {
