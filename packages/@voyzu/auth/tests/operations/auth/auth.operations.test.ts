...
import { authenticateUser } from "@voyzu/auth/auth/operations";
import { createUser, getUser } from "@voyzu/auth/users/operations";
...
const password = "authenticate-operation-password";

before(async () => {
  const admin = await getUser("ADMIN");
  assert.ok(admin, "The seeded ADMIN user is required for authentication operation tests");
...
test("authenticateUser authenticates a user through the public operation", async () => {
  const authenticated = await authenticateUser(code.toLowerCase(), password);
  assert.equal(authenticated.code, code);
  assert.equal(authenticated.displayName, "Authentication operation test");
});
