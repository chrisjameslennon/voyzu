...
export type HttpApiBasicAuthResult =
  | { kind: "none" }
  | { kind: "authenticated"; user: UserResponseDto }
  | { kind: "response"; response: NextResponse };

function unauthorized(message = "Invalid HTTP API credentials"): NextResponse {
  return NextResponse.json(
    ...
    },
    {
      status: 401,
      headers: {
        "WWW-Authenticate": 'Basic realm="Voyzu HTTP API"',
      },
    },
  );
}

function forbidden(message = "User is not enabled for HTTP API access"): NextResponse {
  return NextResponse.json(
    {
      code: "FORBIDDEN",
      message,
...
export async function authenticateHttpApiBasicRequest(request: NextRequest): Promise<HttpApiBasicAuthResult> {
...
export async function withHttpApiBasicAuthContext<T>(
  request: NextRequest,
  callback: () => Promise<T>,
): Promise<T | NextResponse> {
  const auth = await authenticateHttpApiBasicRequest(request);
