...
function toParameters(doc: OperationDoc): OpenApiParameter[] | undefined {
  ...
  const cookieParams = (doc.requestCookies ?? []).map(name => ({
    name,
    in: "cookie" as const,
    required: true,
    schema: { type: "string" },
    ...
    Object.entries(doc.responses).map(([status, response]) => {
      ...
      const setCookieHeader = cookies.length
        ? {
          "Set-Cookie": {
            description: cookies
              .map(name => `${name}: required outgoing cookie (set or clear).`)
              .join(" "),
            schema: { type: "string" },
          },
        }
...
export function generateOpenApi(options: GenerateOpenApiOptions): string {
  ...
  const openApi = {
    openapi: "3.1.0",
    info: {
      title: options.title ?? "Voyzu HTTP API",
      version: options.version ?? "1.0.0",
    },
    tags: JSON.parse(fs.readFileSync(path.join(inputRoot, "tags.json"), "utf8")),
    paths,
  };
