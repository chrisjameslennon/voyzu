import type {
  HttpApiMethod,
  HttpApiOperationData,
  HttpApiResponsePanelData,
  OperationDoc,
...
function toHttpApiMethod(method: OperationDoc["method"]): HttpApiMethod {
...
function buildResponsePanels(doc: OperationDoc): HttpApiResponsePanelData[] {
  return Object.entries(doc.responses).map(([status, response]) => ({
    status: status as HttpApiResponsePanelData["status"],
...
export function operationDocToHttpApiOperationData(doc: OperationDoc): HttpApiOperationData {
  return {
    id: encodeURIComponent(doc.operationId),
    title: doc.summary,
    method: toHttpApiMethod(doc.method),
    path: doc.path,
