import { RequestExample } from "@voyzu/http-api-reference/client/components";
    ...
    <main className={styles.page}>
      <header className={styles.header}>
        ...
        <p className={styles.description}>
          Voyzu supports HTTP API authentication for external callers and local authentication for users working through the
          web application.
        </p>
      </header>

      <section className={styles.section}>
        <h2 className={styles.sectionTitle}>HTTP API Authentication</h2>
        <p className={styles.sectionText}>
          HTTP API calls use HTTP Basic authentication. Construct one exact value as <code>USER_CODE:PASSWORD</code>. The
          ...
          only the password or encode the two parts separately. The user must be active and enabled for HTTP API access.
        </p>
      ...
      <section className={styles.section}>
        ...
        <p className={styles.sectionText}>
          An endpoint can receive either an external HTTP API call or an application call from the Voyzu web app. External
          callers provide authentication credentials in the request. If HTTP API credentials are not received, the endpoint
        ...
        <p className={styles.sectionText}>
          ...
          systems, scripts, integrations, and test clients should use HTTP API authentication. If an Authorization header is
