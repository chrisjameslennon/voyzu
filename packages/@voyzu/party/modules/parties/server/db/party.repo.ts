import { BusinessRuleError, DataError } from "@voyzu/capability/errors";
...
import type { PartyRow, InsertPartyRow, UpdatePartyRow, PatchPartyRow } from "./party.row.types";
...
export class PartyRepo {
  ...
  async insert(row: InsertPartyRow): Promise<PartyRow> {
    const cols: string[] = [];
    const vals: unknown[] = [];
  ...
  async get(code: string): Promise<PartyRow | null> {
  ...
  async update(code: string, row: UpdatePartyRow): Promise<PartyRow> {
    const vals: unknown[] = [];
    ...
    const { rows } = await this.db.query(sql, vals);
    if (!rows[0]) throw new DataError(`Party ${code} not found`);
    return this.mapRow(rows[0]);
  }

  async patch(code: string, updates: PatchPartyRow): Promise<PartyRow> {
    const sets: string[] = [];
    const vals: unknown[] = [];
    ...
    if (sets.length === 0) {
      ...
      if (!existing) throw new DataError(`Party ${code} not found`);
      return existing;
    }
  ...
  async delete(code: string): Promise<void> {
    await this.batchDelete([code]);
  }

  // ── Collection operations ──

  async listAll(): Promise<PartyRow[]> {
    const { rows } = await this.db.query(
  ...
  async filter(filters: Filter[], options?: ListOptions): Promise<PartyRow[]> {
  ...
  async search(phrase: string, options?: ListOptions): Promise<PartyRow[]> {
    const params: unknown[] = [];
  ...
  async batchGet(codes: string[]): Promise<PartyRow[]> {
  ...
  async batchDelete(codes: string[]): Promise<void> {
    ...
    try {
      ...
      // Let foreign keys enforce references without importing consuming packages.
      if (error instanceof Error && ((error as { code?: string }).code === "23503" || error.message.includes("23503"))) {
        throw new BusinessRuleError("A party referenced by other records cannot be deleted");
      }
      throw error;
    }
  }
  ...
  private mapRow(row: Record<string, unknown>): PartyRow {
    return {
      ...row,
      id: Number(row.id),
      creation_date: (row.creation_date instanceof Date ? row.creation_date : new Date(String(row.creation_date))).toISOString(),
      updated_date: (row.updated_date instanceof Date ? row.updated_date : new Date(String(row.updated_date))).toISOString(),
    } as PartyRow;
  }
}
