...
Consider Service A defining a `serviceA.planet` Semantic Data Contract containing the planet's name. Services B and C define extensions describing its continents and seas. Service D combines these into `serviceD.geography`. All four contracts declare `planet` as their entity, allowing the separate contributions to be retrieved together using a shared planet identifier, without duplicating their definitions or owning their data:

```
  Service A defines "serviceA.planet"
      entity: planet
      identifier: planet ID
      fields:
          name

  Service B defines "serviceB.continents"
      entity: planet
      identifier: planet ID
  ...
  Service C defines "serviceC.seas"
      entity: planet
      identifier: planet ID
      extends: "serviceA.planet"
      fields:
  ...
  Service D defines "serviceD.geography"
      entity: planet
      identifier: planet ID
      extends: "serviceA.planet"
      extensions:
...
Or, if Semantic Data Contract names are not required:
...
```
  ...
  Service C defines "serviceC.geography"
      entity: planet
      identifier: planet ID
      extends: "serviceB.continents"
      fields:
          seas

In this way, a hierarchy is established:

  serviceA.planet (entity: planet)
  └── serviceB.continents (entity: planet)
      └── serviceC.geography (entity: planet)
```
...
For example, Service C in our first example could implement `serviceC.seas` for the `planet` entity as follows:

```
Service C implements "serviceC.seas"
    entity: planet
    identifier: planet ID
...
with the corresponding records supplied by those services. However, a request for Service D's composed `serviceD.geography` returns the planet, continents and seas data together.
