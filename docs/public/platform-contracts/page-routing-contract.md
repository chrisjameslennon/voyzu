...
export default {
  contracts: {
    pageRouting: {
      // Each root owns its routes and identifies their navigation area.
      roots: {
        "/finance": {
...
## Roots and navigation selection

`pageRouting.roots` is an object keyed by base URL. Each root contains a `routes` map keyed by globally unique route IDs. Route paths remain full URL patterns and must sit within the root where they are declared. A package may declare multiple roots, each with its own routes and navigation area. A root does not automatically create a page at its base URL or add a top-navigation item.
...
- `routeDefinition`: the entire package route definition, enriched with `id` from its key, `rootPath` from its containing root, `packageName`, `helpBaseUrl` and `httpApiDocsUrl`. Its `helpPath` contains the resolver's result when a resolver is supplied. All other fields, including `loadPage` and `auth`, remain available.
...
- Each route's URL sits within its containing root. For example, a package declaring `/finance` can define `/finance/journals`, but not `/inventory/items`.
...
Combine module route maps within each root with `mergePageRoutes` from `@voyzu/types/page-routing`. It rejects duplicate keys before they can be overwritten. Module exports alone do not register routes; only `contracts.pageRouting` contributes pages.

Use `npm run voyzu:compose -- --routing-only` to refresh page/navigation registries and HTTP documentation without refreshing unrelated composition.
